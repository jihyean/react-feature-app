...
function drag(_) {
    function onDrop(event) {
        ...
        if (rowChangeValid) {
            ...
            const changed = prevFrom != newFrom ||
                prevTo != newTo ||
    ...
    if (!reflected) {
        ...
        selectionManager.taskSettings.set(model.id.toString(), {
            ...
            dragAllowed: () => {
                return ($rowStore.entities[model.resourceId].model.enableDragging &&
                    model.enableDragging);
            },
            resizeAllowed: () => {
                return (model.type !== 'milestone' &&
